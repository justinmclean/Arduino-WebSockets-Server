...
void runCommand(Client client, String command) {
  ...
  // Digital pin ON or OFF D<pin><0|1>
  if (length == 4) {
    if (command.charAt(0) == 'D'
      && (command.charAt(1) == '0' || command.charAt(1) == '1')
      && command.charAt(2) >='0' && command.charAt(2) <= '9'
      && (command.charAt(3) == '0' || command.charAt(3) == '1')) {
      int pin = int(command.charAt(1)-'0')*10 + int(command.charAt(2)-'0');
      boolean on = (command.charAt(3) == '1');
      
      // ADD M command to set mode?
      pinMode(pin, OUTPUT); 
